...
import { storeChatMessage as storePineconeChatMessage } from '@/lib/pinecone/chat-history';
import { storeChatMessage as storeSupabaseChatMessage } from '@/lib/supabase/operations';
...
export async function POST(request: Request) {
  try {
    ...
    // Store conversation in both Pinecone (for embeddings) and Supabase (for persistence)
    try {
      const chatMetadata = {
      ...
      const pineconeId = `chat_${userId}_${Date.now()}`;
      await storePineconeChatMessage(
        userId,
        message,
        responseText,
        chatMetadata
