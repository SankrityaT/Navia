...
export default function ChatInterface({ userContext }: ChatInterfaceProps) {
  ...
  const handleFeedback = async (messageId: string, supabaseMessageId: string | undefined, feedback: boolean) => {
    ...
    try {
      console.log(`📊 Submitting feedback: message_id=${supabaseMessageId}, feedback=${feedback ? 'helpful' : 'not helpful'}`);
      ...
      // Show a temporary error message (could be enhanced with a toast notification)
                ...
                >
                  ...
                  >
                    ...
                    {message.role === 'assistant' && !message.isStreaming && (
                      <div className="mt-4 pt-4 border-t border-gray-100">
                        <div className="text-sm text-gray-600 mb-3">Was this response helpful?</div>
                        <div className="flex gap-2">
                          <button 
                            onClick={() => handleFeedback(message.id, message.supabaseMessageId, true)}
                            disabled={message.userFeedback !== null}
